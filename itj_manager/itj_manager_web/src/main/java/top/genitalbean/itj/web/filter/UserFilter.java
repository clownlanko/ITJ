...
@WebFilter(filterName = "userFilter",urlPatterns = {"/view/editor.jsp"})
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(request.getSession().getAttribute("user")!=null){
           filterChain.doFilter(request,response);
            ...
            content.append("<script src='../js/jquery-2.1.4.min.js'></script>");
            content.append("<script src='../layer/layer.js'></script>");
            content.append("<style>body{margin:0px;}</style>");
            content.append("</head>");
            content.append("<body>");
            content.append("<canvas id='coderain' style='background:#111'></canvas>");
            content.append("<script charset='UTF-8' src='../js/coderain.js'></script>");
