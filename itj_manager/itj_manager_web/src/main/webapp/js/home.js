function queryRankingList() {
    $.get("/ITJ/ac.itj/qr.itj", function (json) {
        for (var i = 0; i < json.data.length; i++) {
            $(".topPannel>ul").append("<li><a href='/ITJ/view/article.jsp?ai=" + json.data[i].articleId + "'>" + json.data[i].title + "</a></li>");
        }
    });
}
var s=0,e=10;
function queryArticles() {
    $.get("/ITJ/ac.itj/ga.itj?s="+s+"&e="+e, function (json) {
        if(json.state==4){
            for (var i = 0; i < json.data.length; i++) {
                var article = $("<div class='article'></div>");
                var title = $("<div class=\"title\"><a href=\"/ITJ/view/article.jsp?ai=" + json.data[i].articleId +
                    "\"><h3>" + json.data[i].title + "</h3></a><strong>" + json.data[i].nickname + "</strong></div>");
...
function seartch(content) {
    $(".seratch>ul>li").remove();
    $.get("/ITJ/ac.itj/acs.itj?keyword=" + content, function (json) {
        for (var i = 0; i < json.data.length; i++) {
            var li = $("<li><a href='/ITJ/view/article.jsp?ai=" + json.data[i].articleId + "' style='margin-right: 50px;'>" + json.data[i].title + "</a>" + json.data[i].nickname + "</li>");
            $(".seratch>ul").append(li);
        }
    });
    $(".seratch>ul").css("visibility", "visible");
}

//画樱花
var RENDERER = {
    INIT_CHERRY_BLOSSOM_COUNT: 5,
    ...
    createCherries: function () {
        for (var i = 0, length = Math.round(this.INIT_CHERRY_BLOSSOM_COUNT * this.width / 10000); i < length; i++) {
...
$(function () {
    ...
    window.onscroll = function () {
        setTimeout(function () {
            ...
            timeoutInt = window.setTimeout(function () { //监听事件内容
